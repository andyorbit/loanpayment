...
export function matchTransactions(sheetRows, pendingPayments, namePatterns, alreadyMatchedTxIds = []) {
  ...
  for (let i = 1; i < sheetRows.length; i++) {
    ...
    const notes = (row[notesIdx] || '').toUpperCase();
    ...
    // Check if any pattern matches Name, Description, or Notes
    const textMatch = namePatterns.some(p => {
      const upper = p.toUpperCase();
      return name.includes(upper) || desc.includes(upper) || notes.includes(upper);
    });
